#!/usr/bin/env python3
"""Export gold tables from Aurora PostgreSQL to CSV + DDL for workshop seeding.

Reads AURORA_DSN from the project-root .env file, connects via psycopg2, and
writes each gold table to a CSV (with geometry as WKT) plus a companion
create_tables.sql file so the tables can be re-created in a fresh Aurora
instance.
...
def main() -> None:
    ...
    # Write DDL file
    ddl_path = SEED_DIR / "create_tables.sql"
    ddl_path.write_text("\n".join(ddl_statements))
